...
class TFliteDemo:
    def __init__(self, model_path, size=(64, 128), blank=0, conf_mode="min"):
        self.size = size
        self.blank = blank
...
if __name__ == '__main__':
    ...
    # img_size = (64, 128)
    # init and load model
    demo = TFliteDemo('demo/f32_ctr_later_s2.tflite', size=img_size)
    ...
    for i in range(len(img_list)):
        ...
        result['image_path'] = os.path.basename(img_list[i].split('_')[0])
    ...
    for result in res_confs:
        if result['image_path'].split('.')[0] == result['label']:
            correct_count += 1
